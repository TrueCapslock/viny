...
export function TastingList({
  ...
  async function deleteTasting(id: number) {
    ...
    if (typeof window !== "undefined" && !window.confirm("Slett denne smakingen? Handlingen kan ikke angres.")) {
      return
    }
    setDeletingId(id)
    try {
          ...
          >
            ...
            {isOpen && hasContent && (
              <div className="px-4 pb-4 pt-0 border-t border-cream-200/80 animate-fade-in">
                 ...
                 * the header, to avoid:
                 *   1) nested <button>-in-<button> HTML abuse (the
                 *      header is rendered as <button onClick=expand>
                 *      in the hasContent branch),
                 *   2) `stopPropagation()` ceremony on mobile where
                 *      a tap on the edit icon would otherwise toggle
                 *      expansion as well.
